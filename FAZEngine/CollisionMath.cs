...
namespace FAZEngine
{
    ...
    static public class CollisionMath
    {
        ...
        /// <summary>
        /// Determines if two circles intersect or contain each other.
        /// </summary>
        ...
        /// <summary>
        /// Determines if a rectangle and line segment intersect, and if so, how they do.
        /// </summary>
        ...
        /// <param name="result">The result data for the collision.</param>
        /// <returns>True if a collision occurs, provided for convenience.</returns>
        public static bool RectangleLineCollide(Vector2[] rectangleCorners, 
            Vector2 lineStart, Vector2 lineEnd, out Vector2 point)
        {
            List<Vector2> hitPoints = new List<Vector2>();
            for (int i = 0; i < 4; i++)
            {
                Vector2 hitPoint = new Vector2();
                if (LineLineIntersect(rectangleCorners[i], rectangleCorners[(i + 1) % 4], lineStart, lineEnd, out hitPoint))
                    hitPoints.Add(hitPoint);
            }

            if (hitPoints.Count > 0)
            {
                // find the point closest to lineStart
                float distance = (hitPoints[0] - lineStart).Length();
                point = hitPoints[0];
                foreach (Vector2 p in hitPoints)
                {
                    if ((p - lineStart).Length() < distance)
                    {
                        distance = (p - lineStart).Length();
                        point = p;
                    }
                }
            }
